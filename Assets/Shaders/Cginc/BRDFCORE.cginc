    ...
    {
        float Fc = MPow5( 1 - VoH );					// 1 sub, 3 mul
    ...
    FSphericalGaussian MakeNormalizedSG(float3 LightDir, half Sharpness)
    {
        ...
        SG.Amplitude = SG.Sharpness / ((2 * UNITY_PI)  * (1-exp(-2 * SG.Sharpness))); //归一化处理
        return SG;
    }
   
    

    float3 SGDiffuseLight(float3 N,float3 L, half3 ScatterAmt)
    {
        FSphericalGaussian red=MakeNormalizedSG(L,1/max(0.0001f,ScatterAmt.x));
        FSphericalGaussian green=MakeNormalizedSG(L,1/max(0.0001f,ScatterAmt.y));
        FSphericalGaussian blue=MakeNormalizedSG(L,1/max(0.0001f,ScatterAmt.z));
