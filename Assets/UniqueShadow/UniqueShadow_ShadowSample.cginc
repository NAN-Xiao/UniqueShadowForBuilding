...
uniform Texture2D _UniqueShadowTextureFakePoint;
uniform SamplerState sampler_UniqueShadowTextureFakePoint;
...
half SampleUniqueD3D11(half4 uv, half4 screen) {
	float dist = _UniqueShadowTextureFakePoint.Sample(sampler_UniqueShadowTextureFakePoint, float2(0,0)).a;
	for(int j = 0; j < 10; ++j) {
		const half2 poi = poisson[j + 24];
		const half2 off = poi * u_UniqueShadowBlockerWidth;
		float depth = _UniqueShadowTexture.Sample(sampler_UniqueShadowTextureFakePoint, uv.xy + off).r;
	...
	for(int i = 0; i < 32; ++i) {
		...
		shadow += _UniqueShadowTexture.SampleCmpLevelZero(sampler_UniqueShadowTexture, uv.xy + off, uv.z);
	} 
...
half SampleUniqueD3D9OGL(const half4 coords) {
	...
	for(int i = 0; i < 8; ++i) {
		...
		shadow += UNITY_SAMPLE_SHADOW(_UniqueShadowTexture, uv.xyz);
	} 
	return shadow / 8.f;
}
...
#define UNIQUE_SHADOW_TRANSFER(o)			                    o.uniqueShadowPos = mul(_UniqueShadowMatrix, float4(posWorld.xyz, 1.f));
...
#define UNIQUE_SHADOW_LIGHT_ATTENUATION(destName, i, woldPos)   (UNITY_LIGHT_ATTENUATION(destName, i, woldPos) * UNIQUE_SHADOW_ATTENUATION(i))
	...
	#define UNITY_TRANSFER_SHADOW(i, coord)			o.uniqueShadowPos = mul(_UniqueShadowMatrix, float4(worldPos.xyz, 1.f));
